#!/usr/bin/env python3
"""Audit the Opus + Sol scaffold-ladder screen's agent-visible views and transcripts.

Same two surfaces and token list as audit_v040_blindness.py (generation 1) and
audit_v040_sol_ablation_blindness.py, scoped to this study's own suite/run ids.
"""
...
def main() -> None:
    ...
    for suite_id in V040_SCAFFOLD_LADDER_SUITE_IDS:
        for transcript in sorted((arguments.submission_root / suite_id).rglob("transcript-attempt-*.stream.jsonl")):
            ...
            # Same interpreter-path allow-list as generation 1 and the sol ablation.
