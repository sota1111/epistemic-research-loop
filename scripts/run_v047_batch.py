#!/usr/bin/env python3
"""Run one v0.4.7 suite's 4 candidates with bounded parallelism.

Same resumable pattern as run_v044_batch.py: shells out to run_v047_agent.py per
(suite, run_id) pair, skipping pairs that already have a recorded agent_submission.json.
"""
...
def main() -> None:
    ...
    pending = [
        run
        for run in V047_CANDIDATE_RUN_IDS
        if not (arguments.output_root / arguments.suite_id / run / "agent_submission.json").exists()
    ]
    print(f"{len(pending)} of {len(V047_CANDIDATE_RUN_IDS)} runs pending", flush=True)
    failures: list[str] = []
    ...
    summary = {"study": "v047-kaggle-late-submission-suite", "pending": len(pending), "failures": failures}
