...
def main() -> None:
    ...
    verdict = ("context 条件付けが優る" if lo > 0 else
               "チェックリストが優る" if hi < 0 else
               "**差は検出できなかった**(区間が 0 をまたぐ)")
    ...
    OUT.write_text(json.dumps({
        ...
        "unobservable_states": [s for s in states
                                if all(c["labels"].get(s, 0) == 0 for c in comps)],
        "global_prior": {s: {str(k): round(v, 4) for k, v in gp_all[s].items()} for s in states},
        "conditional": {s: {",".join(map(str, k)): {str(a): round(b, 4) for a, b in v.items()}
                            for k, v in cond_all[s].items()} for s in states},
