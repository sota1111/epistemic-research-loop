#!/usr/bin/env python3
"""Unblind the locked Opus + Sol scaffold-ladder Stage 2 follow-up and report results.

Reports discovery events, false promotions, and diversity metrics per configuration, as
in Stage 1, plus a named binary endpoint per docs/v040_scaffold_ladder_stage2_
preregistration.json: whether persistent_delayed_history (0 discoveries across 88 prior
runs, docs/v040_discovery_ledger.md) was genuinely discovered by any configuration.
"""
...
def main() -> None:
    ...
    lock = json.loads((arguments.suite_root / "scaffold_stage2_agent_runs.lock.json").read_text())
    expected = len(V040_SCAFFOLD_STAGE2_SUITE_IDS) * len(V040_SCAFFOLD_STAGE2_RUN_IDS)
    if lock.get("all_outputs_locked_before_hidden_evaluation") is not True or lock.get("agent_run_count") != expected:
        raise SystemExit(f"all {expected} scaffold-stage2 outputs must be locked before unblinding")
    ...
    loaded = tuple(
        ...
        for suite_id in V040_SCAFFOLD_STAGE2_SUITE_IDS
        for run_id in V040_SCAFFOLD_STAGE2_RUN_IDS
    )
    report = evaluate_v038_runs(
        tuple(item.base for item in loaded),
        truths,
        None,
        expected_suite_count=len(V040_SCAFFOLD_STAGE2_SUITE_IDS),
    )
    base = report.base
    per_config: dict[str, dict[str, object]] = {}
    for run_id, config in V040_SCAFFOLD_STAGE2_CONFIGS.items():
    ...
    _write(
        arguments.output_root / "v040_scaffold_stage2_selection.json",
        {
            "version": "0.4.0",
    ...
    _write(
        arguments.output_root / "v040_scaffold_stage2_diagnostics.json",
        {
