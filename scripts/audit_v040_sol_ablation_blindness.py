#!/usr/bin/env python3
"""Audit the codex sol reasoning-effort ablation's agent-visible views and transcripts.

Same two surfaces and token list as audit_v040_blindness.py (generation 1), scoped to
the ablation's own suite/run ids.
"""
...
def main() -> None:
    ...
    for suite_id in V040_SOL_ABLATION_SUITE_IDS:
        for transcript in sorted((arguments.submission_root / suite_id).rglob("transcript-attempt-*.stream.jsonl")):
            ...
            # Same interpreter-path allow-list as generation 1 (docs/verification/
            # v040_gen1_track_a_qualification.md): `which python3`/RuntimeWarnings name the
            # venv symlink, not repository content. A real repository path still fails.
