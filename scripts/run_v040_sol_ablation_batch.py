#!/usr/bin/env python3
"""Run the codex sol reasoning-effort ablation (12 runs) with bounded parallelism.

Each (suite, run) pair is executed by `scripts/run_v040_agent.py` as its own fresh
codex process. Completed pairs (a recorded agent_submission.json) are skipped, so the
...
def main() -> None:
    ...
    print(json.dumps({"study": "sol-effort-ablation", "pending": len(pending), "failures": failures}, indent=2))
    if failures:
        raise SystemExit(1)
