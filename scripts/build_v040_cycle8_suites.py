#!/usr/bin/env python3
"""Build and lock the six suites for the cycle-budget ablation (4 -> 8 cycles per pack).

Independent side-probe: raises max_cycles_per_pack to 8 for Opus x P1 and Sol x P1 x
xhigh, comparing against each configuration's existing cycle=4 baseline from prior
studies rather than re-running it. Resumable, matching the other v0.4.0 suite builders.
...
from epistemic_loop.benchmark.v040_grammar_suite import (
    V040_CYCLE8_CONFIGS,
    V040_CYCLE8_MASTER_SEED,
    V040_CYCLE8_MAX_CYCLES_PER_PACK,
    V040_CYCLE8_RUN_IDS,
...
def main() -> None:
    ...
    lock_path = arguments.output_root / "cycle8_suite_set_lock.json"
    ...
    if lock_path.exists():
        ...
        new_suite_ids = [suite_id for suite_id in V040_CYCLE8_SUITE_IDS if suite_id not in previous_results]
        if not new_suite_ids:
            raise SystemExit("v0.4.0 cycle8 suite set already covers every preregistered suite id")
    ...
    prompt_paths = {
        ...
        "p1c8": Path("prompts/generic_research_agent/v040_p1_c8.md"),
    }
    policy_contract = {
        "null_policy": {
            "full_refit": True,
    ...
    for suite_id in V040_CYCLE8_SUITE_IDS:
        ...
        result = build_v040_suite(
            ...
            suite_ids=V040_CYCLE8_SUITE_IDS,
            master_seed=V040_CYCLE8_MASTER_SEED,
            configs=V040_CYCLE8_CONFIGS,
            run_ids=V040_CYCLE8_RUN_IDS,
            max_cycles_per_pack=V040_CYCLE8_MAX_CYCLES_PER_PACK,
        )
    ...
    payload = {
        ...
        "max_cycles_per_pack": V040_CYCLE8_MAX_CYCLES_PER_PACK,
        "suite_ids": list(V040_CYCLE8_SUITE_IDS),
        "execution_configurations": {run: dict(config) for run, config in V040_CYCLE8_CONFIGS.items()},
        "runs_per_suite": len(V040_CYCLE8_RUN_IDS),
        "total_runs": len(V040_CYCLE8_SUITE_IDS) * len(V040_CYCLE8_RUN_IDS),
        "replicates_per_configuration": len(V040_CYCLE8_SUITE_IDS),
