#!/usr/bin/env python3
"""Build one v0.4.4 full-feature pilot agent view for a preregistered CompetitionSpec.

See docs/verification/v044_full_feature_pilot_preregistration.md. Single run, not a
batch: this is a feasibility pilot, not a scaled study.
"""
...
from epistemic_loop.benchmark.v044_full_feature_pilot import V044_MASTER_SEED, build_v044_pilot
...
def main() -> None:
    ...
    parser.add_argument("--prompt-path", type=Path, default=Path("prompts/generic_research_agent/v044_pilot.md"))
    ...
    if arguments.lock_file.exists():
        raise SystemExit(f"pilot already locked at {arguments.lock_file}; delete deliberately to rebuild")
