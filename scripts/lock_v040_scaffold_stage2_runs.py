#!/usr/bin/env python3
"""Validate and hash-lock the Opus + Sol scaffold-ladder Stage 2 follow-up's 18 runs."""
...
from epistemic_loop.benchmark.v040_grammar_suite import V040_SCAFFOLD_STAGE2_RUN_IDS, V040_SCAFFOLD_STAGE2_SUITE_IDS
...
def main() -> None:
    ...
    lock_file = arguments.suite_root / "scaffold_stage2_agent_runs.lock.json"
    if lock_file.exists():
        raise SystemExit("v0.4.0 scaffold-stage2 outputs are already locked")
    pairs = [(suite, run) for suite in V040_SCAFFOLD_STAGE2_SUITE_IDS for run in V040_SCAFFOLD_STAGE2_RUN_IDS]
    ...
    print(json.dumps({"locked": True, "study": "scaffold-ladder-stage2", "runs": len(records)}, indent=2))
