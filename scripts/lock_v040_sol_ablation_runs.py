#!/usr/bin/env python3
"""Validate and hash-lock the codex sol reasoning-effort ablation's 12 runs."""
...
from epistemic_loop.benchmark.v040_grammar_suite import V040_SOL_ABLATION_RUN_IDS, V040_SOL_ABLATION_SUITE_IDS
...
def main() -> None:
    ...
    lock_file = arguments.suite_root / "sol_ablation_agent_runs.lock.json"
    if lock_file.exists():
        raise SystemExit("v0.4.0 sol-ablation outputs are already locked")
    pairs = [(suite, run) for suite in V040_SOL_ABLATION_SUITE_IDS for run in V040_SOL_ABLATION_RUN_IDS]
    ...
    print(json.dumps({"locked": True, "study": "sol-effort-ablation", "runs": len(records)}, indent=2))
