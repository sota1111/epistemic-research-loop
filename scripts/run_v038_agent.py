#!/usr/bin/env python3
"""Execute one fresh-context v0.3.8 agent run through the `claude -p` CLI.
...
- Write your final output to `agent_submission.json` in this directory with `version` `0.3.8`.
...
def main() -> None:
    ...
    parser.add_argument("--output-root", type=Path, default=Path(".runs/v038/agent_outputs"))
    parser.add_argument("--workdir-root", type=Path, default=Path.home() / "erl-v038-runs")
...
def _validate(submission_path: Path, packet: dict[str, object]) -> tuple[str, ...]:
    ...
    return validate_v038_submission(loaded, packet).errors
