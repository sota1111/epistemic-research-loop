#!/usr/bin/env python3
"""Unblind the locked v0.3.9 qualification batch and write the full report set."""
...
from epistemic_loop.controller.v039_agent import load_v039_submission
...
def main() -> None:
    ...
    if not lock.get("all_outputs_locked_before_hidden_evaluation") or lock.get("agent_run_count") != 24:
        raise SystemExit("all 24 qualification outputs must be locked before v0.3.9 unblinding")
    ...
    truths = tuple(
        decrypt_v037_suite(arguments.truth_root / f"{suite_id}.manifest.enc", key) for suite_id in V039_QUAL_SUITE_IDS
    )
    loaded = tuple(
        load_v039_submission(arguments.submission_root / suite_id / run_id / "agent_submission.json")
        for suite_id in V039_QUAL_SUITE_IDS
        for run_id in V039_RUN_IDS
    )
    ...
    _write(
        arguments.output_root / "v039_agent_reproducibility_scorecards.json",
        {
    ...
    _write(
        arguments.output_root / "v039_population_blind_spot_report.json",
        {
    ...
    _write(
        arguments.output_root / "v039_structure_failure_traces.json",
        {
    ...
    _write(arguments.output_root / "v039_full_refit_null_audit.json", asdict(report.provenance_audit))
    _write(
        arguments.output_root / "v039_qualification_result.json",
        {
            "version": "0.3.9",
...
def _verify_lock(lock: dict[str, object], suite_root: Path, submission_root: Path) -> None:
    ...
    expected = {(suite_id, run_id) for suite_id in V039_QUAL_SUITE_IDS for run_id in V039_RUN_IDS}
...
def _verify_suite_set_lock(lock: dict[str, object], truth_root: Path) -> None:
    if lock.get("qualification_suite_ids") != list(V039_QUAL_SUITE_IDS):
