...
3. ~~**`−1` を再現できる形にする。**~~ **実施した。**決定性と必要性を分けた 2 部モデルを
   事前登録([coding_rules_v2_counterexample.md](coding_rules_v2_counterexample.md))して当てはめ、
   **自分の事前登録基準で落ちた**([結果](results_counterexample.md))。
   反例 0/6、一致率 −0.0081、対数損失は区間が 0 をまたがず**悪化**。
   原因は上の 2 と同じ穴 —— **`−1` の 6 セルは全部 cv / nlp にあり、そこで context 特徴が定数**。
   採ったのは**分布に `P(−1)` を持たせる形**で、点予測へ潰さない限り反例は残る
4. ~~**NEDO へ適用する**~~ **予測を事前登録した**([prediction_nedo.md](prediction_nedo.md))。
   **そこで分かったこと:corpus は 8 つの context セルのうち 6 つしか観測しておらず、
   NEDO のセル `(0,1,1)` は空である。**よって条件付き分布は全体分布へ落ち、
   **NEDO に対する「context 条件付けの予測」はチェックリストと 1 ビットも違わない。**
   NEDO でテストできるのはチェックリストのほうで、context 条件付けの当否は測れない。
   点予測は **RMC のみ `+1`、他 12 状態は 0**
