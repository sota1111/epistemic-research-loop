...
**2026-09-06 に [handover.md](handover.md) を最小構成へ差し替えた。この文書は保管である。**
v0.3.x〜v0.5.0 の経過が入っているが、**現行の正本ではない。**
現在地と次の一手は [handover.md](handover.md) と
[ERL を完成させるための引き継ぎ書](v050_course_correction.md) を見ること。
...
`.runs/` に置いていたエージェント成果物が dev container 再ビルドで全消失した。
**コミットされていた docs だけが残った。**以後、成果物は必ずリポジトリに残す。

| リポジトリ | 内容 |
| --- | --- |
| [erl-nedo-baggage-loading](https://github.com/sota1111/erl-nedo-baggage-loading) | SIGNATE NEDO コンテスト2(**主軸**、締切 2026-10-19、賞金1500万円) |
| [erl-ieee-cis-fraud](https://github.com/sota1111/erl-ieee-cis-fraud) | IEEE-CIS(計画のみ、未実行) |
| [erl-santander-ctp](https://github.com/sota1111/erl-santander-ctp) | Santander(計画のみ、未実行) |
| [erl-kaggriculture](https://github.com/sota1111/erl-kaggriculture) | Kaggle kaggriculture(**並走**、締切 2026-09-30、賞金 $50,000、§5) |

いずれも **1エージェント = 1 `agent/*` ブランチ**。`main` にコンペ正本とラウンド計画を置き、
エージェントがそれを読む。**盲検プロトコルは意図的に廃止した**——v0.3.7〜v0.4.8 の
qualification 結果とは直接比較できず、「独立に構造を再発見した」という主張は今後できない。

環境の永続化: `/workspaces/epistemic-research-loop` はホストにマウントされておらず消えた。
`/home/ubuntu/dev/2026/epistemic-research-loop` へ移して bind mount を
`ai-dev-control-plane` の devcontainer.json に追加済み(PR #426)。

## 1. NEDO コンテスト2 ラウンド1 の結果

問題クラスが根本的に違う: **組合せ最適化 + PyBullet 物理シミュレーション**で、提出物は
予測値 CSV ではなく **Python のアルゴリズム**。ERL の Suite 機構(sealed transfer、
CANONICAL_FEATURES、Matched Negative、盲検監査)は一切使わない。継承したのは
「population × 設計軸 → 機械的選定 → 実スコアで親選定」というループ構造だけ。

**13候補を sealed 12問で採点し、8件を実提出。**

| run | model | arm | ローカル | **実スコア** |
| --- | --- | --- | ---: | ---: |
| r1-05 | claude-opus-5 | **P1** | 60.13 | **60.386** |
| r1-17 | **claude-fable-5.1** | P3 | 59.96 | 58.206 |
| opusfinal(修正) | claude-opus-5 | P3 | 56.27 | 58.203 |
| r1-07 / evo8cache | gpt-5.6-sol xhigh | P3 | 49.76 | 57.847 |
| opus 中断版 | claude-opus-5 | P3 | 64.78 | 56.583 |
| r1-08 | gpt-5.6-sol xhigh | P3 | 21.09 | 39.738 |
| opusfinal(バグ) | claude-opus-5 | P3 | 60.85 | **0** |

**知見:**

1. **モデル軸が支配的。** claude 系5体が 56.06〜64.78 に密集、codex sol 5体は 15.0〜49.8。
   **分布が重ならない。**ただし**実スコアでは差が 2.5 ポイントに縮む**(60.386 対 57.847)
   ——ローカルが差を誇張していた。
2. **P3 優位は転移しなかった。** opus 内で P1 60.13 対 P3 56.06/56.27。sol でも P1 平均が上。
   表形式コンペの「P3 が構造発見のゲート(22/22 P3 / 0/14 P1)」はこの問題クラスでは成立しない。
3. **実行間ばらつきの大きさ自体がモデル依存。** opus-P3 は幅 8.7、sol-xhigh は幅 34.8。
   **単一 run の比較で軸の効果を語ってはならない**(v0.4.3-f の教訓の再現)。
4. **`effort=low` は使用不能。** 2体とも 12/12 でゲート未達、reference 近傍。
5. **fable が opus と互角**(58.206 対 60.386)。opus クォータが有限なので実務的に重要。
6. **9体すべてが同じ解法族に収束した**(高さマップ + 貪欲 + validator 自前再実装)。
   オフライン bin packing ソルバ・探索木・学習はゼロ。参照実装を捨てた fable も同族に着地。

## 2. 採点系で見つけた欠陥(本ラウンド最大の成果)

**すべて「ローカル指標が実は別の量を測っていた」型で、IEEE-CIS の失敗と同型。**

1. **配布 `evaluator.py` が接地した荷物を充填率から落とす。** `env.py` が validator の
   `inclusion_margin = -0.005` を渡すため、床に接地した荷物は内積 +9.2e-06 で
   「はみ出し」と判定される。実測 fill 0.00 → margin +0.01 で 2.83。
2. **私の揺動試験が厳しすぎて `stability_score` が恒常 0 だった。** 0.30g×3周で平均変位
   0.271m(基準 0.10m)。**4指標のうち1つが定数を返す状態で順位付けしていた。**
   0.20g×2周×60 に較正。
3. **課題ジェネレータが `index` を位置と一致させ、要件(5)「再計算」を一度も生成しなかった。**
   この2つの穴により、位置と index を取り違えたエージェントを**原理的に検出できなかった**
   ——ローカル 12/12 完走・composite 66.05 のまま **実スコア 0**。事前積載があると
   index が非連番になり `set_item_order` が False、`app.py` が `break` して評価全体が落ちる。
   1行修正で **0 → 58.203**。

> **教訓: 課題セットが張っていない軸は、エージェントの弱点ではなく採点系の盲点になる。**

## 3. ランナーで見つけた欠陥

1. **プロンプトを argv で渡すと `pgrep -f` が自分にマッチする。** エージェントが
   後片付けに `for p in $(pgrep -f "tools.local_scorer"); do kill -9 $p; done` を実行し、
   **自分自身を SIGKILL した**(2時間45分ぶんが消失)。プロンプトを `PROMPT.md` へ移し
   argv は1行に。v0.4.3-f の `ps -ef` 盲検リークと同じ根。
2. **`simulator/` を丸ごとコピーすると他個体の解が子から見える。** 除外パターン `r1-*` は
   `_score/r1-08/` のようなサブディレクトリに一致せず素通りした。**除外方式をやめ許可制に。**
...
| 予測子 | RMSE | 傾き |
| --- | ---: | ---: |
| `num_placed_items` 単独 | **1.87** | 0.729 |
| 私の composite(5指標合成) | 2.54 | 0.447 |
| `fill_score` 単独 | 4.79 | 0.770 |

**積載率という単一の数字が、5指標の合成より実スコアをよく予測する。**傾きが全て 1 未満
= ローカルは差を 2 倍以上に誇張している。ゲート閾値を振ると RMSE は 0.5→2.54 /
0.4→2.38 / 0.3→4.80 / なし→6.96 で、**崖は存在し閾値は 0.4〜0.5 付近**。
n=6 では重み(未知数5)を分離できず、無制約に当てはめると傾きが負になる(過学習)。

**次:** ゲート閾値のプローブ(`r1-05` から積載率を人為的に制限した変種を投げる)を先に
完了させてからラウンド2を起動する。**採点系が変わるなら、変えてから最適化させる。**

ラウンド2の設計(親選定は §5.3 を実スコアへ機械適用済み):

| | run | model | arm | 実スコア |
| --- | --- | --- | --- | ---: |
| 親1 | `r1-05` | claude-opus-5 | P1 | 60.386 |
| 親2 | `r1-17` | claude-fable-5.1 | P3 | 58.206 |
| 親3 | `evo8cache` | gpt-5.6-sol xhigh | P3 | 57.847 |

進化ランナー(`--parent` / `--findings`)は実装・検証済み。子は各親から opus×2・sol×2、
アームは P1×1 + P3×1。マージ材料は `r1-02`(切り欠き帯が使える機序、15mm 密着不能の
構造的上限)と `r1-08`(棚上52mm浮かしで lift を強制)。


## 5. Kaggriculture 参戦の判断(2026-09-03)

**結論: `kaggriculture` に参戦する。`biohub-cell-tracking-during-development` は見送る。**
判断基準はユーザー指定の「メダル獲得確率が高い方」。以下はすべて 2026-09-03 時点の
Kaggle API とフル LB スナップショットから算出した実測値。
...
| 金(24位 / 16位) | 2820.6 | 0.947 |
| 現順位 | 3,253位(897.2) | 2,254位(0.810) |
| 提出形式 | `tar.gz`(エージェント)5回/日 | **kernels-only**(GPU ノートブック)|
| 見積り(銅) | **30〜40%**(当初 70〜80% から下方修正、§5.5) | **10〜20%**(公開解の丸ごと採用でも 30% 程度) |

### 5.1 決め手 — 銀圏のエージェントを既に作ってあり、それを失効させていた

提出履歴の全件確認で、`kaggriculture-claude` の champion **moon-198**(`main.py`
sha256 `7c5a4a2a…`、198行、SOT-2368/2417)が **2026-08-03 に実 LB 2543.4** を記録していた。
現在の LB に当てはめると **165位 = 上位 2.23%(銀圏)**。銅ライン 1957.2 に対し 585 ポイントの余裕。

**このコンペの LB は「最高スコア」ではなく「現在アクティブなエージェント」を表示する。**
自ベスト 2543.4 があるのに順位が 3,253位(897.2)である事実がその証拠であり、
**2543.4 は失効していた。**8/3 以降 claude リネージを止め gpt リネージだけを回した結果、
gpt は同時期 104〜264、最終 897.2 で止まっている——**NEDO ラウンド1 の
「claude 系と codex/sol 系の分布が重ならない」という知見(§1-1)がここでも再現している。**

### 5.2 Biohub を採らない理由

- 自ベスト 0.810 から銅 0.934 まで **1,922 チーム分**の差。`biohub-claude` は 113 PR・
  9 サイクルを回して 0.509 → 0.810、直近サイクルは全 REJECT で頭打ち。
- **LB が公開ノートブックの台地で埋まっている**(0.9360 に 132 チーム、0.9350 に 79、
  0.9340 に 57、0.9330 に 89、0.9150 に 150)。**銅の境界が台地の内側にある**ため、
  締切までに台地は銅ラインの下へ押し下げられる。つまりメダルには公開解を上回る必要があるが、
  現在地は公開解より 0.126 下。同着ブロックは提出時刻順で末尾に付くため、いま丸ごと採用しても
  終盤には抜かれる。
- kernels-only の 3D 光シート動画に対し、手元の GPU は RTX 3080 Ti 1枚(12GB)。残り 26 日では届かない。

### 5.3 Kaggriculture 固有のリスク(既知、対処込み)

1. **スコアは相対評価。** 600 が相対スコアのノイズ床(SOT-2379: soil は実 LB で 600 に落ちた)。
   8/3 の 2543.4 は当時のフィールドに対する値で、現在の首位は 2961.8 まで伸びている。
   → **moon-198 の再提出そのものを「現フィールドに対する再測定」として最初に実施した(§5.4)。**
2. **ローカル評価が信用できない。** SOT-2417 で、公開上位5体のリプレイで組んだ replay-league が
   **実 LB の順序を反転させる**ことが実証済み(実 LB では moon-198 ≫ soil、league では逆)。
   IEEE-CIS の相関逆転・NEDO の採点系欠陥と**同型**の失敗(ローカル指標が別の量を測っている)。
   → **信頼できるゲートは実 LB のみ。**5回/日 × 27日 = 約 135 回の実測予算があり、
   これは v0.4.8 の「実スコアで親選定」ループを回すには十分。ローカル league は昇格判定に使わず、
   明らかな破綻の足切りに用途を限定する。
3. **champion の出自は公開カーネルの蒸留。** moon-198 は公開カーネル
   `prvsiyan/kaggriculture-frontier-the-moon-counts-melons`(episode 89674601)の蒸留であり、
   独自成果ではない。`opponents/MANIFEST.json` に 5 体分の `kernel_ref` / sha256 / 抽出方式を記録済み。
   **ERL で積み上げる分はこのベースラインからの上積みとして測る。**
4. **NEDO との資源競合はない。** CPU シミュレーションのみで GPU 不要、ランナーとループは共通。
   ただし締切は NEDO(10/19)より先に来る(9/30)。

### 5.5 訂正(2026-09-03 02:32)— 「銀圏の個体がある」は成立しなかった

**§5.1 の決め手は、moon-198 の再提出による再測定で否定された。**
ユーザーの指摘(「コンペが始まったばかりのときに提出したので上ブレしたのでは」)が正しかった。

- **コンペ開始は 2026-07-30 16:15 UTC。2543.4 は開始4日目の値**で、54エピソードすべてが
  08-03 の7時間以内に消化されている。
- **同一バイト列の再提出は 13エピソードで 967.0。**相手平均レーティング 828 に対し通算 7勝4敗、
  **同格帯(828〜1013)では 2勝4敗。**2500級の個体なら 900 帯を圧倒するはずだが、していない。
- moon-198 の蒸留元カーネルの作者 prvsiyan は 9/02 提出のアクティブなチームで、
  現在 **1,084位 / 1,685.7 と銅ライン 1957.2 を下回る**。方策族ごと追い越されている。

**したがって銅 70〜80% の見積りは撤回する。**レーティングは相対値であり、
**8月のレーティングを9月の分布に載せた対応づけ(2543.4 → 165位=銀圏)は無効**だった。
§5.3-1 でこれをリスクとして挙げながら、確率見積りにはその仮定を組み込んだまま使った
——**これが本判断の最大の誤りであり、IEEE-CIS・NEDO と同じ「ローカルな数字が実は
別の量を測っていた」型の失敗の、3例目である。**今回は測る対象が
「他時点のレーティング」だった。

**再見積り:** moon-198 の実力は 900〜1100級、銅まで 2,500チーム分。**銅 30〜40%。**

**それでも kaggriculture を選ぶ結論は変わらない。**根拠が差し替わる:
公開解の台地がメダルラインを塞いでいない / 問題クラスがループと一致 /
相手の replay から上位方策を直接観測できる / ドーマント層が分母をかさ上げしている。
Biohub 側の評価は変わらないため相対順位は維持されるが、**絶対的な期待値は大きく下がった。**
ラウンド1 の昇格基準は moon-198 ではなく**銅ライン 1957.2** に、撤退条件は
「D7 で 1,500(上位20%)に届かなければ NEDO へ資源を寄せる」に書き換えた。

### 5.4 実施済みと次の一手

- **moon-198 を再提出済み**(2026-09-03、submission `55971967`)。受理時 600.0 → 13エピソードで
  967.0。**2543.4 は再現しなかった(§5.5)。**この値がラウンド1の親スコアになる。
- 以後の開発は **ERL で行う**。新リポジトリ [erl-kaggriculture](https://github.com/sota1111/erl-kaggriculture)
  の `main` にコンペ正本・評価基盤・ラウンド計画を置き、**1エージェント = 1 `agent/*` ブランチ**
  (NEDO と同じ規約)。`kaggriculture-claude` / `kaggriculture-gpt` の auto-improve 系統は凍結。
- gpt / sol リネージは打ち切り。両コンペとも claude 系に対して劣位が実測で確定している。
