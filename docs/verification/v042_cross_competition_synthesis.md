...
広げると、IEEE-CIS では全く別の2件の新規 unclassified パターンが独立に出現し、
うち1件(seed402)は Santander の当初の脱出claim(seed17)と構造的にほぼ同一の
「加法性の限界」を主張していた**(発見15)——単一 seed の novelty は信頼できないが、
**十分な population サイズがあれば、コンペを跨いで繰り返し現れる新しいメタ技術候補
(taxonomy 未収載の第3候補)を見出せる**、という、より頑健な形で未知構造探索の
価値が確認された。

**方法論的な副産物:** (a) `SD-high-P3`(IEEE-CIS)・`SD-high-P1`(Santander)は
いずれも n=8 中 6/8 が P2 を達成する頑健な sol 単独構成として確定した。(b) 8 seed目に
して Santander 側で初めて FSPR 汚染が1件観測され、Controller 側の独立検証が正しく
機能することを実地で確認した(発見17)。(c) バッチオーケストレータの起動引数経由の
盲検リーク(`ps -ef`)を発見・修正し、新しい不変条件として記録した——suite_id だけで
なく、エージェントバッチを起動する全てのコマンドの引数がコンペ名を含まないよう
徹底する必要がある。
...
2. **技術クラス taxonomy は、コンペ固有の具体的技術(列意味論依存)とデータ形式非依存の
   ...
   (「加法性の限界」、v0.4.3-f で2件独立観測)も今後の確認対象に加える。
