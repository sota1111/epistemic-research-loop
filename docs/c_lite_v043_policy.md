...
**⚠️ 訂正(2026-08-30):** v0.4.3-f(§10、sol reasoning-effort 多様性ラウンド)は、
ユーザーの承認を得ずに継承されていた固定10列制約の下で実施されたことが判明した——
詳細は[10列制約インシデント記録](verification/v044_ten_column_constraint_incident.md)。
v0.4.3-f の結論(diversity 指数・layer1/layer2 一致率等)は「10列制約下でのみ妥当」
として保持し、全特徴量設計での再実施計画は [c_lite_v044_policy.md](c_lite_v044_policy.md)
を参照。v0.4.3-a/b/d(§1・2・4)は列数に依存しない検証であり、この訂正の対象外。
